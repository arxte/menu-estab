...
// const basketPreview = ref<HTMLDivElement | null>()
// const minusDishBtn = ref<Array<HTMLButtonElement> | null>()
// const priceBtn = ref<HTMLDivElement | null>()
// const imgDishInBasket = ref<HTMLDivElement | null>()
// const amountImgDishInBasket = ref<HTMLDivElement | null>()f
...
<template>
  <div class="card-dish-wrapper">
    <article
      class="card-dish"
      v-for="dish of dishesStore.sweetBreakfast"
      :key="dish.id"
    >
      <div class="card-dish__img-wrapper">
        ...
        <div
          ...
          v-if="dishesStore.breakfastDishes[dish.id].amount > 0"
        >
          <span class="card-dish__img-amount-dish">{{ dishesStore.breakfastDishes[dish.id].amount }}</span>
        </div>
        <img
        :src="`${dish.imagePath}`"
          :alt="dish.title"
      ...
      <div class="card-dish__content">
        ...
        <div class="card-dish__price-wrapper">
          <div
            :class="['card-dish__price', { add_active: dishesStore.breakfastDishes[dish.id].amount > 0 }]"
            ref="priceBtn"
          >
            <button
              type="button"
    ...
    <!-- <article
      class="basket-preview-wrapper"
      ref="basketPreview"
      v-if="dishesStore.countDishesInBasket != 0"
    >
      <router-link to="/basket" class="basket-preview">
        <span class="basket-preview__title">в корзине</span>
        <span class="basket-preview__cost"
          >{{ dishesStore.countDishesInBasket }} {{ dishesStore.wordOnBtn }} на
          {{ dishesStore.costDishesInBasket }}</span
        >
      </router-link>
    </article> -->
  </div>
